  ...
  return (
    <div className="min-h-screen pt-24 pb-16 px-4 bg-white">
      <div className="max-w-3xl mx-auto">
        <h1 className="text-4xl font-bold mb-6">Email the team</h1>

        <form onSubmit={handleSubmit} className="space-y-4 bg-white rounded-2xl p-8 shadow"> 
          <div>
          ...
          <div>
            ...
            <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" className="w-full px-4 py-3 rounded-xl border border-gray-300" required />
          </div>
          ...
          <div className="flex gap-3">
            <button type="submit" disabled={status === 'sending'} className="px-6 py-3 bg-unmakt-2 text-white rounded-xl font-semibold">
              {status === 'sending' ? 'Sending...' : 'Send Email'}
            </button>
            <button type="button" onClick={() => { setName(''); setEmail(''); setPhone(''); setMessage(''); }} className="px-6 py-3 bg-gray-100 rounded-xl">
              Clear
            </button>
          </div>
        </form>
